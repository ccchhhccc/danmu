$(function(){
	...
	var tobarHtml = `<li><a href="http://localhost:2255/html/usermain.html?${u_id}">主页</a></li>
					 <li class="active"><a href="http://localhost:2255/html/usercenter.html?${u_id}">关注</a></li>
	...
	//拼接下面的列表
	var html = ''
	for(var i in himlike){
		var flag = false
		for(var j in mylike){
			if(himlike[i].u_id==mylike[j].u_id){
				flag = true
				break
			}
		}
	...
	if(userinfo.liker==1 && u_id!=sessionStorage.getItem("userid")){
		$('.himlike').html('')
	}
